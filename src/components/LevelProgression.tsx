...
/*    const levelOne = data!.data[0].data

    const levelOneUnlockedAt = levelOne.unlocked_at as Date
    const levelOnePassedAt = levelOne.passed_at as Date
    const durationLevelOneInDays = (new Date(levelOnePassedAt).getTime() - new Date(levelOneUnlockedAt).getTime()) / 1000 / 60 / 60 / 24

    console.log("durationlevelOne", durationLevelOneInDays)*/
    ...
    return (
        <div>
            ...
            <Bar data={barData} type={"bar"} options={options} plugins={[annotationPlugin, ChartDataLabels]}/>
        </div>
    )
}
